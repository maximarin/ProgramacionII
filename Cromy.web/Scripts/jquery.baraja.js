...
; (function ($, window, undefined) {
    ...
    $.Baraja.prototype = {
        ...
        _init: function (options) {
            ...
            this.$playerName = this.$el.parent().parent().parent().find('.player-name');
            this.$cardCount = this.$el.parent().parent().parent().find('.card-count');
            this.$buttons = this.$el.parent().parent().parent().find('.button-area button');
            this.$items = this.$el.children('li');
            this.itemsCount = this.$items.length;
            this.$cardCount.html(this.itemsCount);
        ...
        _add: function ($elems) {
            ...
            self.$cardCount.html(self.itemsCount);

            // set z-indexes
            self._setStack($elems);
        ...
        _remove: function () {
            ...
            var cardToRemove = self.getFirstCard().find(".front-card");
            self.getFirstCard().remove();

            // reset
            self.$items = this.$el.children('li');
            self.itemsCount = this.$items.length;
            self.$cardCount.html(this.itemsCount);
            self.order();
        ...
        getFirstCard: function () {
            return this._getFirstCard();
        },
        order: function () {
            this.$items = this.$items.sort(function (a, b) {
                // Ordeno de mayor z-index a menor.
                return parseInt($(b).css('z-index'))
                    - parseInt($(a).css('z-index'));
            });
        },
        setWinner: function () {
            var self = this;
            self.$el.parent().find('.star-1').fadeOut(150).delay(1200).fadeIn(300).fadeOut(150).delay(1254);
            self.$el.parent().find('.star-2').fadeOut(300).fadeIn(120).fadeOut(120).delay(1920);
            self.$el.parent().find('.star-3').fadeOut(150).delay(1000).fadeIn(300).fadeOut(150).delay(800);
            self.$el.parent().find('.star-4').fadeOut(700).fadeIn(300).fadeOut(160).delay(1350);

            self.$playerName.addClass('winner');

            setTimeout(function () {
                self.$playerName.removeClass('winner');
            }, 2000);
        },
        rotateCard: function () {
            this.$el.find(".card-container").removeClass("flip");
            this.getFirstCard().addClass("flip");
        },
        ...
        addWonCard: function (wonCard, isActualPlayer) {
            var self = this;

            var newItem = $('<li></li>');

            if (!isActualPlayer) {
                $(newItem).addClass('card-container');

                $(wonCard).addClass('front-card');

                newItem.append(backCard);
            }

            self.add(newItem.append(wonCard));
            ...
            setTimeout(function () {
                self.$el.find(".card-container").removeClass("flip");
                self.order();
                self.adjustCards();
            }, 2500);
        },
